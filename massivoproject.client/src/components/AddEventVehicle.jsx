...
import {
    Box, TextField, Button, Typography, Paper, Grid, IconButton, Tooltip, MenuItem
} from '@mui/material';
...
const AddVehicleEvent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        showAlert('Por favor ingrese un teléfono válido antes de enviar', 'error');
        return;
    }
        ...
        if (eventDate) {
            ...
            if (salida > evento) {                
                showAlert('La fecha de salida no puede ser posterior a la fecha del evento.', 'error');
                setLoading(false);
                return;
            }
        }
    ...
    return (
        <Box sx={{ backgroundColor: '#F5F5F5', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Paper elevation={6} sx={{ p: 4, borderRadius: 4, width: { xs: '95vw', md: 500 } }}>
                <Typography variant="h5" fontWeight="bold" mb={2} color="#139AA0">Agregar Vehículo a Evento</Typography>
                <form onSubmit={handleSubmit}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}><TextField label="Fecha de salida" type='date' name="fechaSalida" value={form.fechaSalida} onChange={handleChange} fullWidth required InputLabelProps={{ shrink: true }}
                            inputProps={{ max: maxDate }} /></Grid>

                        <Grid item xs={12}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                <TextField label="Lugar de partida" name="lugarPartida" value={form.lugarPartida} onChange={handleChange} fullWidth required />
                        ...
                        <Grid item xs={12}><TextField label="Hora de partida" type="time" name="horaPartida" value={form.horaPartida} onChange={handleChange} fullWidth required /></Grid>
                        <Grid item xs={12}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                <TextField
                                    label="Destino"
                        ...
                        <Grid item xs={12}><TextField label="Hora de regreso" type="time" name="horaRegreso" value={form.horaRegreso} onChange={handleChange} fullWidth required /></Grid>
                        <Grid item xs={12}><TextField label="Lugar de regreso" name="lugarRegreso" value={form.lugarRegreso} onChange={handleChange} fullWidth required /></Grid>
                        <Grid item xs={12}><TextField label="Teléfono" name="telefono" value={form.telefono} onChange={handleChange} fullWidth required  /></Grid>

                        <Grid item xs={12}><TextField label="Precio por pasajero" type='number' name="precioPorPasajero" value={form.precioPorPasajero} onChange={handleChange} fullWidth required /></Grid>
                        <Grid item xs={12}><TextField label="Observaciones" name="observaciones" value={form.observaciones} onChange={handleChange} fullWidth multiline minRows={3} /></Grid>
                    </Grid>
                    <Grid item xs={12}>
                        <Button
                            type="submit"
                            variant="contained"
                            fullWidth
                            sx={{ mt: 2, backgroundColor: "#139AA0", ":hover": { backgroundColor: "#11787c" } }}
                        >
                            Crear Evento
                        </Button>
                    </Grid>
